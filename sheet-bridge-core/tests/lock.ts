...
describe("lock program", () => {
  ...
  it("user can send valid tokens to this contract; event emitted", async () => {
    ...
    const userPre = (await provider.connection.getTokenAccountBalance(userTokenAccount)).value.amount;
    const vaultPre = await safeGetTokenAmount(provider.connection, vaultTokenAccount);

    const sig = await program.methods
      .lockTokens(new BN(lockAmount))
      .accounts({
        user: user.publicKey,
        lockAccount: lockAccountPda,
        mint,
    ...
    const userPost = (await provider.connection.getTokenAccountBalance(userTokenAccount)).value.amount;
  ...
  it("invalid tokens rejected; no event emitted", async () => {
    ...
    const badUserToken = (
      await getOrCreateAssociatedTokenAccount(provider.connection, user, badMint, user.publicKey)
    ).address;
