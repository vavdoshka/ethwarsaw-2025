...
function parseTokensLockedFromLogs(
    logs: string[]
): { sender: PublicKey; amount: BN; recipient: string } | null {
    ...
    for (const line of logs) {
        ...
        if (rest.length < 32 + 8 + 4) continue;
        const sender = new PublicKey(rest.subarray(0, 32));
        const amount = new BN(rest.subarray(32, 40), 'le');
        const strLen = rest.readUInt32LE(40);
        if (rest.length < 44 + strLen) continue;
        const recipient = rest.subarray(44, 44 + strLen).toString('utf8');
